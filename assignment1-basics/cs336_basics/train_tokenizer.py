...
def chunk_processer(
    ...
    with open(input_path, "rb") as f:
        ...
        split_chunk_list: list[str] = re.split(
            f"({special_token_pattern})", file_chunk
        )
        # GPT-2 regex
        ...
        for chunk in split_chunk_list:
            if chunk in special_tokens:
                count_map[chunk] = count_map.get(chunk, 0) + 1
                continue
...
def run_train_bpe(input_path: str, vocab_size: int, special_tokens: list[str]):
    ...
    with Pool(num_processes) as pool:
        count_maps: list[dict[str, int]] = pool.starmap(
            chunk_processer,
            [
        ...
        for map in count_maps:
            for pretoken in map:
                final_count_map[pretoken] = (
                    final_count_map.get(pretoken, 0) + map[pretoken]
                )
    ...
    def train_bpe_pretokenization(count_map: dict[str, int], num_merges: int):
        ...
        for i in range(num_merges):
            ...
            if not counts:
                break
            pair = max(
                counts, key=lambda x: (counts[x], vocab[x[0]], vocab[x[1]])
            )
            max_index1, max_index2 = pair
            new_index = 256 + len(special_tokens) + i
            merges.append((vocab[max_index1], vocab[max_index2]))
            vocab[new_index] = vocab[max_index1] + vocab[max_index2]
    ...
    return train_bpe_pretokenization(
        final_count_map, vocab_size - 256 - len(special_tokens)
    )
